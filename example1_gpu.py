...
def PINNtrain(train_data, epochs, model):
    
    dist_dataset = mirrored_strategy.experimental_distribute_dataset(train_data)
    ...
    diffusion_x, grad_diffusion_x = diffusion_fcns(np.double(train_data.numpy()))
    exact_x = exact_solu(np.double(train_data.numpy()))
    exact_grad_x, exact_div_x = exact_grad_div(np.double(train_data.numpy()))
    
    # Main training loop
    for i in range(epochs):
        
        epoch_loss[i], N_loss[i], gradient_loss[i], divergence_loss[i]  = trainStep_N_dimension(dist_dataset, diffusion_x, grad_diffusion_x, exact_x, exact_grad_x, exact_div_x, opt, model)
...
np.savetxt('example1.out', (PDE_loss1,N_loss1))
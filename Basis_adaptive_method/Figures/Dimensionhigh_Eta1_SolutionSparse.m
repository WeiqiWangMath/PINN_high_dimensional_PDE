...
%     m(i) = Rand_index(i);
%     c(i) = rand(1);
%     u_temp = @(x) c(i) * ones(size(x(:,1)));
%     for k = 1 : n
%         if I(k,m(i)) ~= 0
%             u_temp = @(x) u_temp(x) .* sin(2*pi* I(k,m(i)) *x(:,k));
%         end
%     end
%     u_exact = @(x)  u_exact(x) + u_temp(x);
%     
% end
% save('data/D8_Sparse_solution.mat','u_exact','m','I')
...
y_data = zeros(size(s_vals,2),N_runs,3);
...
save('data/D8_Eta1_SoluSparse.mat','s_vals','Sp','y_data','N');

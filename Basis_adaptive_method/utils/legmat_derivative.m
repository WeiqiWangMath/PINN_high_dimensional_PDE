...
if d >= 1
    A1 = zeros(length(grid),k); % A1: Legendre 1st-derivative matrix
    A1(:,1) = 0;
    A1(:,2) = 1;
    for i = 2:k-1
        A1(:,i+1) = i * A(:,i) + A1(:,i) .* grid;
    end
end
if d == 2
    A2 = zeros(length(grid),k); % A2: Legendre 2nd-derivative matrix
    A2(:,1) = 0;
    A2(:,2) = 0;
    for i = 2:k-1
        A2(:,i+1) = i * A1(:,i) + A1(:,i) + A2(:,i) .* grid;
    end
end
if d == 1
    A = A1;

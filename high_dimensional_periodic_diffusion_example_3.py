...
Example 1: 
    exact solution: $\Phi_{exact}(x) = \sum_{k=1}^{p} c_k \sin{2\pi m_{1k} x_{d_{1k}}}\sin{2\pi m_{2k} x_{d_{2k}}}$
    p is number of terms, c_k are random real number in [0,1], m_{1k} and m_{2k} are random integers
List of arguments:
    ...
    #7 : p - number of terms in the exact solution
...
np.savetxt('data/' + file_name + 'terms' + str(p) +'_Run' +str(N_runs)+ '.out', (PDE_loss1, N_loss1, error_epoch))